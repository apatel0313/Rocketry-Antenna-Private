...
def main():
    ...
    initial_angle = math.degrees(math.atan(initial_alt / GROUND_STATION_FROM_POLE))
    initial_pulse = rest_pulse - deg_step_pulse * initial_angle
    
    # Send the servo to its true starting position

    
    # Pre-load our tracking variables
    ...
    try:
        while True:
            ...
            # FIX 2: Use the perfectly smooth interpolated angle directly for position.
            angle_deg = round(math.degrees(theta), 2)
            
            # Only use the polynomial if we explicitly need derivatives
            if current_coeffs is not None and COMPUTE_DIAGNOSTICS:
                _, omega, alpha = compute_angle_and_derivatives(
                    time_from_launch, 
            ...
            if abs(angle_deg - last_angle) >= 0.15:
                if now - last_servo_time >= servo_interval:
                    ...
                    if COMPUTE_DIAGNOSTICS:
                        print_str = (f"T:{time_from_launch:05.2f} | Alt:{altitude:5.1f}m | "
                                     f"Angle:{commanded_angle:05.2f}° | Omega:{math.degrees(omega):.2f}°/s | "
                                     f"Torque:{torque:+.4f}Nm [{torque_status}] ***")
                    else:
                        print_str = (f"T:{time_from_launch:05.2f} | Alt:{altitude:5.1f}m | "
                                     f"Angle:{angle_deg:05.2f}° → {commanded_angle:05.2f}° Pulse:{pulse:.1f} ***")
